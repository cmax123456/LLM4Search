...
def parse_dataset_args(parser):
    ...
    parser.add_argument("--min_pos_per_query", type=int, default=2,
                        help="minimum number of positive (E/S) docs per query; queries below this are filtered out")
...
def parse_test_args(parser):
    ...
    parser.add_argument("--test_datasets", type=str, default="game.test,zhihu.test,qa.test",
                        help="comma-separated list of test dataset prefixes")
...
def load_datasets(args):
    ...
    if task.lower() == "seqrec":
        ...
        first_test_prefix = [x.strip() for x in test_datasets_str.split(",")][0]
        valid_data = RetrievalDataset(args, "test", args.valid_prompt_sample_num, test_prefix=first_test_prefix)

    return train_data, valid_data

def load_test_dataset(args, test_prefix=None):

    print(args.test_task)
    if args.test_task.lower() == "seqrec":
        test_data = SeqRecDataset(args, mode="test", sample_num=args.sample_num, test_prefix=test_prefix)
    elif args.test_task.lower() == "retrieval":
        print(f"Loading retrieval test dataset with test_prefix: {test_prefix}")
        test_data = RetrievalDataset(args, mode="test", sample_num=args.sample_num, test_prefix=test_prefix)
    else:
        raise NotImplementedError

    return test_data

def prefix_allowed_tokens_fn(candidate_trie, input_length=0):
    def prefix_allowed_tokens(batch_id, sentence):
        sentence = sentence.tolist()[input_length:]
