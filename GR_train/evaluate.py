...
def get_metrics_results(topk_results, generated_results, target, targets_with_rel, metrics):
    ...
    for m in metrics:
        ...
        elif m.lower().startswith("topk_hit"):
            k = int(m.split("@")[1])
            res[m] = topk_hit_k(generated_results, k, target)
        else:
...
def topk_hit_k(generated_results, k, target):
    score = 0.0
    for n, row in enumerate(generated_results):
        gt_topk = target[n][:k]
        hit_count = sum(1 for doc in gt_topk if doc in row)
        score += hit_count / min(k, len(target[n])) if len(target[n]) > 0 else 0
    return score
