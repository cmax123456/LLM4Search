...
class ConEmbDataset(data.Dataset):
    ...
    def __init__(self, data_path):
        ...
        # 动态推导目录路径，去上一级目录查找
        file_path = os.path.dirname(os.path.dirname(data_path)) + "/"
    ...
    def _sample_contrastive_pair(self, index):
        ...
        if index in self.doc2query:
            random_query = random.choice(self._doc2query_keys[index])
            q_emb = self.query_embeddings[random_query]
            c_rel = self.doc2query[index][random_query]
            inner_contrastive_doc = index
            candidates = [d for d in self._query2docs_keys[random_query] if d != index]
            if candidates:
                inner_contrastive_doc = random.choice(candidates)
            ...
            sam_rel_doc = index
            same_rel_candidates = [d for d in self.query2docs_rel[random_query][c_rel] if d != index]
            if same_rel_candidates:
                sam_rel_doc = random.choice(same_rel_candidates)
            rel_list = [1,2,3]
    ...
    def _create_query_embs(self):
        ...
        for query in self.query2docs:
            ...
            if len(doc_list) > 0:
                self.query_embeddings[query] = calculate_average_embedding(self.embeddings[doc_list])
            else:
                self.query_embeddings[query] = np.zeros(self.dim, dtype=np.float32)
    ...
    def __getitem__(self, index):
        ...
        if extra_index:
            extra_tensor_emb = self.embeddings[[i[0] for i in extra_index]]
    ...
    def _load_query_info(self, file_path):
        ...
        self._doc2query_keys = {doc: list(queries.keys()) for doc, queries in self.doc2query.items()}
        self._query2docs_keys = {query: list(docs.keys()) for query, docs in self.query2docs.items()}
